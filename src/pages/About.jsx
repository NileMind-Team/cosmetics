...
import { Building, Users, Target, CheckCircle, HardHat } from "lucide-react";
...
const About = ({ lang }) => {
  ...
  const content = {
    en: {
      ...
      description:
        "With over 15 years of experience, Saaed Wafi Construction delivers high-quality, reliable, and innovative projects across residential, commercial, and industrial sectors.",
      tabs: [
    ...
    ar: {
      ...
      description:
        "بخبرة تزيد عن 15 عامًا، تنفذ شركة سعد وافي مشاريع سكنية وتجارية وصناعية بأعلى معايير الجودة والالتزام.",
      tabs: [
  ...
  return (
    ...
    >
      <div className="container mx-auto px-6 text-center max-w-4xl">
        {/* <div className="inline-flex items-center gap-2 bg-[#0056B3] text-white px-3 py-1.5 rounded-full text-xs mb-3">
          <HardHat className="w-3.5 h-3.5" />
          {lang === "ar" ? "شركة مقاولات معتمدة" : "Certified Construction Company"}
        </div> */}
            ...
            <button
              ...
              className={`flex items-center gap-1.5 px-4 py-1.5 rounded-full border text-sm transition ${
                activeTab === i
                  ...
                  : "bg-transparent text-gray-600 border-gray-300 hover:border-[#FF7A00]"
              }`}
            >
              {tab.icon}
              {tab.title}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-8">
          {current.stats.map((stat, i) => (
            <div
              key={i}
              className="bg-gradient-to-br from-[#0056B3] to-[#003d82] text-white p-4 rounded-lg text-center"
            >
        ...
        <div className="mt-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
            {current.why.map((item, i) => (
              <div
                key={i}
                className="flex items-center gap-2 bg-white dark:bg-gray-800 p-2 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 text-sm"
              >
